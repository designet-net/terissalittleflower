	...
	if(is_admin() && current_user_can('activate_plugins') && !wp_doing_ajax()){
		...
		if(empty($softaculous_pro['license']['license'])){
			...
			if(!empty($softaculous_pro['license']['url_mismatch'])){
				$msg = sprintf(__('Your SoftWP plugin license is <b><font style="color:red;">not authorized</font></b> to be used on  <b><i>%1$s</i></b> or you have reached the maximum number of sites allowed for this license. Please upgrade your license <a href="%2$s" target="_blank">here</a>.', 'softaculous-pro'), 
							site_url(),
							SOFTACULOUS_PRO_BUY
							);
